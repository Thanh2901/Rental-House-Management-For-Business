...
export default class ApiService {
  ...
  static async loginUser(loginData) {
    try {
      const resp = await axios.post(
        `${this.BASE_URL}/auth/access-token`,
        loginData
      );
      ...
      if (
        ...
        if (userInfo.data.data && userInfo.data.data.credentialId) {
          ...
          console.log(
            "Không tìm thấy credentialId trong phản hồi từ /auth/user-info"
          );
        }
        ...
        console.error(
          "Không có accessToken hoặc refreshToken trong phản hồi:",
          resp.data
        );
      }
      return resp.data;
  ...
  static async logoutUser() {
    ...
    try {
      const resp = await axios.post(
        `${this.BASE_URL}/auth/remove-token`,
        {},
        {
          headers: {
            ...this.getHeader(),
  ...
  static async forgetPassword(email) {
    const resp = await axios.post(
      `${this.BASE_URL}/auth/forget-password`,
      email
    );
    return resp.data;
  }
  ...
  static async registerUser(registrationData) {
    ...
    console.log("🚀 ~ ApiService ~ registerUser ~ resp:", resp);
  ...
  static async getRoomTypes() {
    try {
      ...
      if (Array.isArray(resp.data)) {
        ...
        throw new Error(
          "Unexpected response format: Expected an array of room types"
        );
      }
    } catch (error) {
      console.error(
