...
function Navbar() {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        isOpen &&
        !event.target.closest(".navbar-menu-container") &&
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      if (resp && resp.code === 200) {
        ...
        toast.error(
          "Đăng xuất thất bại: " + (resp?.message || "Lỗi không xác định"),
          {
            position: "top-right",
            autoClose: 3000,
          }
      ...
      toast.error(
        "Lỗi đăng xuất: " + (error.message || "Không thể kết nối đến server"),
        {
          position: "top-right",
          autoClose: 3000,
        }
